...
def save(data, path: str, format: str = "auto", verbose: bool = True, **kwargs):
    ...
    if isinstance(data, pd.DataFrame):
        savers = {
            ...
            ".tsv": lambda d, p, **kw: d.to_csv(
                p, sep="\t", index=kw.get("index", False)
            ),
            ".json": lambda d, p, **kw: d.to_json(
                p, orient=kw.get("orient", "records")
            ),
            ".parquet": lambda d, p, **kw: d.to_parquet(
                p, index=kw.get("index", False)
            ),
