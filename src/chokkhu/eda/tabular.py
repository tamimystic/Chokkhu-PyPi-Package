...
class TabularEDA:
    ...
    def _analyze_data(self) -> None:
        ...
        self.results['numerical_cols'] = self.df.select_dtypes(include=['int64', 'float64']).columns.tolist()
        self.results['categorical_cols'] = self.df.select_dtypes(include=['object', 'category']).columns.tolist()
    ...
    def _visual_reports(self) -> None:
        ...
        if self.results['missing'].sum() > 0:
            ...
            sns.heatmap(self.df.isnull(), cbar=False, cmap='viridis', yticklabels=False, ax=ax)
            ax.set_title("Missing Values Heatmap")
            PlotVisualizer.save_and_show(fig, "1_missing_values.png", self.save_dir, self.save_reports)

        # 2. Correlation Matrix
        if 'correlation' in self.results and not self.results['correlation'].empty:
            fig, ax = plt.subplots(figsize=(12, 10))
            sns.heatmap(self.results['correlation'], annot=True, cmap='coolwarm', fmt=".2f", linewidths=0.5, ax=ax)
            ax.set_title("Feature Correlation Matrix")
            PlotVisualizer.save_and_show(fig, "2_correlation_matrix.png", self.save_dir, self.save_reports)

        # 3. Numerical Distributions
        num_cols = self.results['numerical_cols']
        if num_cols:
            num_plots = min(len(num_cols), 6) # Plot up to 6 numerical columns to avoid clutter
            fig, axes = plt.subplots(int((num_plots + 1) / 2), 2, figsize=(15, 5 * int((num_plots + 1) / 2)))
            ...
            PlotVisualizer.save_and_show(fig, "3_numerical_distributions.png", self.save_dir, self.save_reports)

        # 4. Categorical Counts
        cat_cols = self.results['categorical_cols']
        if cat_cols:
            cat_plots = min(len(cat_cols), 4)
            fig, axes = plt.subplots(int((cat_plots + 1) / 2), 2, figsize=(15, 6 * int((cat_plots + 1) / 2)))
            axes = axes.flatten() if cat_plots > 1 else [axes]
            
            for i, col in enumerate(cat_cols[:cat_plots]):
                sns.countplot(data=self.df, y=col, ax=axes[i], palette='Set2', order=self.df[col].value_counts().index[:10])
                axes[i].set_title(f"Top Categories in {col}")
            
            for j in range(cat_plots, len(axes)):
                axes[j].axis('off')
                
            PlotVisualizer.save_and_show(fig, "4_categorical_counts.png", self.save_dir, self.save_reports)
