...
class ImageEDA:
    ...
    def _analyze_data(self) -> dict:
        ...
        """
        exts = ('.png', '.jpg', '.jpeg')
        counts = {os.path.basename(p): len([f for f in os.listdir(p) if f.lower().endswith(exts)]) for p in self.class_paths}
        ...
        return {
            "df_counts": df_counts, "sizes_df": df_sizes, "total_images": processed_count,
            "avg_blur": np.mean(blur_scores) if blur_scores else 0,
            "avg_rgb_hist": avg_hist, "total_classes": len(self.class_paths)
        }

    def _visual_reports(self):
        """
        ...
        for i, path in enumerate(self.class_paths[:9]):
            ...
            plt.subplot(3, 3, i+1)
            plt.imshow(img); plt.title(os.path.basename(path)); plt.axis("off")
        plt.suptitle("Sample Images per Class", fontsize=15); plt.tight_layout(); plt.show()

        # 2. Size Analysis (Boxplot + Aspect Ratio Hist)
        ...
        ax2.set_title("Aspect Ratio Distribution"); plt.show()

        print("\n" + "-"*45 + "\nIMAGE SIZE DESCRIPTIVE STATISTICS\n" + " "*45)
        print(res["sizes_df"].describe()); print("-"*45)

        # 3. Scatter Plot & RGB
        plt.figure(figsize=(8, 5))
        plt.scatter(res["sizes_df"]["Width"], res["sizes_df"]["Height"], alpha=0.4, color='teal')
        plt.title("Width vs Height Scatter Plot"); plt.xlabel("Width"); plt.ylabel("Height"); plt.show()
        ...
        plt.title("Global Average RGB Intensity Distribution"); plt.legend(); plt.show()

        # 4. Final Summary
        max_c = res["df_counts"].loc[res["df_counts"]["Image_Count"].idxmax()]
        min_c = res["df_counts"].loc[res["df_counts"]["Image_Count"].idxmin()]
        summary = {
            "EDA Aspect": ["Total Classes", "Total Images", "Majority Class", "Minority Class", "Avg Aspect Ratio", "Avg Blur Score"],
            "Observation": [res["total_classes"], res["total_images"], f"{max_c['Class']} ({max_c['Image_Count']})", 
                            f"{min_c['Class']} ({min_c['Image_Count']})", round(res["sizes_df"]["Aspect_Ratio"].mean(), 2), round(res["avg_blur"], 2)]
        }
        print("\n" + "-"*60 + "\nCOMPLETE DATASET EDA SUMMARY\n" + " "*60)
        print(pd.DataFrame(summary).to_string(index=False)); print("-"*60 + "\n")